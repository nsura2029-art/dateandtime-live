...
(function () {
  ...
  function initDropdowns() {
    ...
    items.forEach((item) => {
      ...
      // Hover to open — small delay so passing over doesn't trigger
      item.addEventListener("mouseenter", () => {
        if (hoverTimer) clearTimeout(hoverTimer);
        hoverTimer = setTimeout(() => open(item), 80);
      });
      item.addEventListener("mouseleave", () => {
        if (hoverTimer) clearTimeout(hoverTimer);
        // Close with small delay so user can move to dropdown content
        hoverTimer = setTimeout(() => {
          if (openItem === item) close(item);
        }, 200);
      });

      // Click on caret toggles open/close
      ...
      // Click on the LABEL navigates to the parent page (don't preventDefault).
      // But also open the dropdown if user holds Shift, for power users.
      if (label) {
  ...
  // ====================================================================
  // SCROLL FADE-IN — Intersection Observer for [data-animate] elements
  // Fades in section blocks as they enter the viewport. CSS handles the
  // animation; this just toggles .is-visible.
  // ====================================================================
      ...
      <div class="continue-strip-inner">
        ...
        <div class="continue-strip-grid">
          <a href="/" class="continue-strip-card">
            ...
            <h3>Today</h3>
            <p>What day is it? Your local time, week, and day of year.</p>
          </a>
          <a href="/holidays/" class="continue-strip-card">
            <span class="continue-strip-icon" aria-hidden="true">🎉</span>
            <h3>Holidays</h3>
            <p>Public holidays for 200+ countries, with long weekend finder.</p>
          </a>
          <a href="/onthisday/" class="continue-strip-card">
            <span class="continue-strip-icon" aria-hidden="true">📜</span>
            <h3>On this day</h3>
            <p>Historical events that happened today, from 1,000 BC to today.</p>
          </a>
          <a href="/world-time/" class="continue-strip-card">
            <span class="continue-strip-icon" aria-hidden="true">🌍</span>
            <h3>World time</h3>
            <p>Live time in 33,945 cities across 250 countries.</p>
          </a>
          <a href="/news/" class="continue-strip-card">
            <span class="continue-strip-icon" aria-hidden="true">📰</span>
            <h3>News</h3>
            <p>Latest time zone changes, DST shifts, and clock news.</p>
          </a>
          <a href="/meeting/" class="continue-strip-card">
            <span class="continue-strip-icon" aria-hidden="true">📅</span>
            <h3>Meeting planner</h3>
            <p>Find a meeting time that works across time zones.</p>
          </a>
        </div>
      </div>
    `;
